...
public class ResultsParser {
   ...
   private ResultsParser.TAGS currentCDATAParent = ResultsParser.TAGS.UNKNOWN;
   ...
   private enum TAGS {
      ...
      public static ResultsParser.TAGS fromString(String val) {
         if (val == null) {
            return UNKNOWN;
         }
         val = val.toUpperCase().replace('-', '_');
         try {
            return ResultsParser.TAGS.valueOf(val);
      ...
      for (FilePath path : paths) {
         ...
         try {
            ...
            while (XmlPullParser.END_DOCUMENT != xmlPullParser.nextToken()) {
               ResultsParser.TAGS tag = ResultsParser.TAGS.fromString(xmlPullParser.getName());
                  ...
                  case XmlPullParser.START_TAG:
                     switch (tag) {
                        ...
                        case PARAMS:
                           ...
                           currentCDATAParent = ResultsParser.TAGS.PARAMS;
                           break;
                        case EXCEPTION:
                           startException();
                           break;
                        case MESSAGE:
                           currentCDATAParent = ResultsParser.TAGS.MESSAGE;
                           break;
                        case SHORT_STACKTRACE:
                           currentCDATAParent = ResultsParser.TAGS.SHORT_STACKTRACE;
                           break;
                        case FULL_STACKTRACE:
                           currentCDATAParent = ResultsParser.TAGS.FULL_STACKTRACE;
                           break;
                     }
                     break;
                  // all closing tags
                  case XmlPullParser.END_TAG:
                     switch (tag) {
                        ...
                        case PARAMS:
                           ...
                           currentCDATAParent = ResultsParser.TAGS.UNKNOWN;
                           break;
                        case EXCEPTION:
                           finishException();
                           break;
