package com.redhat.engineering.jenkins.report.plugin.utils;
...
import com.redhat.engineering.jenkins.report.plugin.results.ClassResult;
import com.redhat.engineering.jenkins.report.plugin.results.MethodResult;
...
public class TestResultHistoryUtil {
   ...
   public static TestResults getPreviousBuildTestResults(AbstractBuild<?, ?> owner) {
      ...
      if (previousBuild != null
               && previousBuild.getAction(ReportPluginBuildAction.class) != null) {
         return previousBuild.getAction(ReportPluginBuildAction.class).getResults();
      } else {
    ...
    * @return
    */
   public static String toSummary(ReportPluginBuildAction action) {
      ...
      if (methodResults != null && methodResults.size() > 0) {
         for (MethodResult methodResult : methodResults) {
            ...
            if (methodResult.getParent() instanceof ClassResult) {
               ...
               htmlStr.append("/").append(owner.getProject().getAction(ReportPluginProjectAction.class).getUrlName());
